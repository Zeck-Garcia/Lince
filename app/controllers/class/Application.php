...
class Application{
    ...
    public function loadListDepartamento(){
        $select = "SELECT * FROM tbDepartamento ORDER BY tbDepartamento.nomeDepartamento";
    ...
    public function loadListNivel(){
        $select = "SELECT * FROM tbClasse ORDER BY tbClasse.nomeClasse";
        $qry = $this->db->buscar($select);
        $list = [];
        if(count($qry) > 0){
            foreach($qry as $value){
    ...
    public function loadListResposanvel(){
        $select = "SELECT * FROM tbUserSistema WHERE aceitarOrderCompra = 1 AND ativoUserDados = 1 ORDER BY nomeUserDados ASC";
        $qry = $this->db->buscar($select);
        $list = [];
        if(count($qry) > 0){
            foreach($qry as $value){
                $list[] = [
                    ...
                    "codClass" => $value["codColaboradorUserDados"],
                    "slug" => $value["idLogin"],
                    ...
                    "receber" => $value["receberEmailUserDados"],
                    "ativo" => $value["ativoUserDados"],
                    "aceita" => $value["aceitarOrderCompra"],
                ];
            }
        }
        return $list;
    ...
    public function loadListCargo(int $id){
        $select = "SELECT * FROM `tbCargo` WHERE departamentoCargo = ? ORDER BY tbCargo.nomeCargo";
    ...
    public function searchListArquivo(int $int, int $type){
        $where = "";
        $param = [];
        $param[] = $int;
        if($type != 0){
            $where = " AND tipoArquivo=? ";
            $param[] = $type;
        }
